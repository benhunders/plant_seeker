...
export default function HomePage() {
  ...
  async function identify(image: string) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Could not identify the plant.');
  ...
  if (stage.kind === 'loading') {
    return (
      <div className="flex flex-col items-center gap-4 py-10">
        ...
        />
        <p className="animate-pulse text-leaf-600">Identifying your plant…</p>
      </div>
    );
  }
      ...
      return (
        <div className="flex flex-col items-center gap-4 py-10 text-center">
          ...
          <p className="text-leaf-700">
            {result.message || "I couldn't confidently identify a plant here."}
          </p>
          <button
            onClick={() => setStage({ kind: 'idle' })}
            className="rounded-xl bg-leaf-500 px-5 py-2.5 font-semibold text-white hover:bg-leaf-600"
  ...
  return (
    <div className="flex flex-col gap-6 py-6">
      <div className="text-center">
        <h1 className="text-2xl font-bold text-leaf-800">What plant is this?</h1>
        <p className="mt-1 text-leaf-600">
          Snap a photo to identify it and get simple care tips.
        </p>
      </div>
      <CameraCapture onCapture={identify} />
      <p className="text-center text-xs text-leaf-500">
        Tip: fill the frame with a single plant in good light.
      </p>
    </div>
  );
}
